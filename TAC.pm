...
sub emit_TAC {
	...
	match ($node) {
		with (Program $declarations) {
			return ::TAC_Program([ map { emit_TAC_($_) } @$declarations ]);
		}
		...
		with (Return $exp) {
			push(@$instructions, ::TAC_Return(emit_tacky($exp)));
		}
		...
		with (Unary $op $exp) {
			...
			my $src = emit_TAC_($exp, $instructions);
			my $dst = ::TAC_Variable(temp_name());
			push @$instructions, ::Unary($unop, $src, $dst);	
			return $dst;
		}
	}
}
